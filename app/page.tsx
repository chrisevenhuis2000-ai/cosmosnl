...
const FALLBACK_ARTICLES: Article[] = [
  ...
  { slug: 'komeet-c2026-a1', title: 'Komeet C/2026 A1 mogelijk zichtbaar met blote oog', excerpt: 'Astronomen zijn enthousiast over een heldere komeet die in april zichtbaar wordt.', category: 'Sterrenkijken', catColor: '#d4a84b', bgColor: 'linear-gradient(135deg,#1a1505,#2a2010)', author: 'Redactie', date: '3 mrt 2026', readTime: 3, featured: false },
...
function Topbar() {
  ...
  return (
    <div role="banner" style={{ position: 'relative', zIndex: 30, height: 'var(--topbar-h)', background: 'rgba(7,8,13,0.97)', borderBottom: '1px solid #1c2035', display: 'flex', alignItems: 'center', gap: 20, backdropFilter: 'blur(12px)' }} className="topbar-pad">
      <span suppressHydrationWarning className="topbar-date" style={{ fontFamily: 'var(--font-dm-mono)', fontSize: '0.58rem', letterSpacing: '0.15em', color: '#4a5278', textTransform: 'uppercase', whiteSpace: 'nowrap', flexShrink: 0 }}>{date}</span>
...
function SiteNav() {
  ...
  return (
    <>
        ...
        <div className="nav-pad" style={{ maxWidth: 'var(--max-w)', margin: '0 auto', height: '100%', display: 'flex', alignItems: 'center', gap: 40 }}>
          ...
          <ul className="nav-links" role="list" style={{ gap: 32, flex: 1, justifyContent: 'center', listStyle: 'none', margin: 0, padding: 0 }}>
            {navLinks.map(({ href, label }) => (
              <li key={href}>
                <Link href={href} style={{ fontSize: '0.68rem', fontWeight: 600, letterSpacing: '0.1em', textTransform: 'uppercase', color: '#4a5278', textDecoration: 'none', transition: 'color 0.15s', padding: '8px 0' }}
                  onMouseEnter={e => (e.currentTarget.style.color = '#dde2f0')}
                  onMouseLeave={e => (e.currentTarget.style.color = '#4a5278')}
                >{label}</Link>
              </li>
            ))}
            <li><Link href="/tools/herschrijver" style={{ fontSize: '0.68rem', fontWeight: 600, letterSpacing: '0.1em', textTransform: 'uppercase', color: '#d4a84b', textDecoration: 'none' }}>AI Tools</Link></li>
          </ul>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexShrink: 0 }}>
            <Link href="/nieuwsbrief" className="btn-clip-sm" style={{ background: '#d4a84b', color: '#07080d', fontFamily: 'var(--font-dm-mono)', fontSize: '0.62rem', fontWeight: 700, letterSpacing: '0.12em', textTransform: 'uppercase', padding: '9px 20px', textDecoration: 'none', display: 'inline-block', transition: 'background 0.15s' }}
            ...
            <button className="nav-hamburger" aria-expanded={mobileOpen} aria-controls="mobile-nav" aria-label={mobileOpen ? 'Menu sluiten' : 'Menu openen'} onClick={() => setMobileOpen(o => !o)} style={{ flexDirection: 'column', gap: 5, padding: 8, background: 'none', border: 'none', cursor: 'pointer' }}>
              {[0, 1, 2].map(i => (
                <span key={i} style={{ display: 'block', width: 22, height: 2, background: '#7a86a8', borderRadius: 1, transition: 'transform 0.25s, opacity 0.25s', transform: mobileOpen ? i === 0 ? 'rotate(45deg) translate(5px,5px)' : i === 2 ? 'rotate(-45deg) translate(5px,-5px)' : 'none' : 'none', opacity: mobileOpen && i === 1 ? 0 : 1 }} />
              ))}
            </button>
          </div>
        </div>
      ...
      {mobileOpen && (
        <div id="mobile-nav" role="navigation" aria-label="Mobiele navigatie" style={{ position: 'fixed', top: 'calc(var(--topbar-h) + var(--nav-h))', left: 0, right: 0, background: 'rgba(7,8,13,0.98)', borderBottom: '1px solid #1c2035', backdropFilter: 'blur(20px)', padding: '24px', zIndex: 19, display: 'flex', flexDirection: 'column', gap: 4, animation: 'fadeIn 0.2s ease both' }}>
          {[...navLinks, { href: '/tools/herschrijver', label: 'AI Tools' }, { href: '/nieuwsbrief', label: 'Nieuwsbrief' }].map(({ href, label }) => (
            <Link key={href} href={href} onClick={close} style={{ display: 'block', padding: '12px 0', fontSize: '0.75rem', fontWeight: 600, letterSpacing: '0.1em', textTransform: 'uppercase', color: '#7a86a8', borderBottom: '1px solid #1c2035', textDecoration: 'none', transition: 'color 0.15s' }}
              onMouseEnter={e => (e.currentTarget.style.color = '#dde2f0')}
  ...
  return (
    <section aria-labelledby="hero-title" style={{ position: 'relative', zIndex: 1, minHeight: '88vh', display: 'flex', alignItems: 'flex-end', overflow: 'hidden' }}>
      <div style={{ position: 'absolute', inset: 0, background: '#0c0e18' }}>
        ...
        {!apod && <div style={{ width: '100%', height: '100%', background: 'linear-gradient(135deg,#0c0e18 0%,#111a2a 50%,#0a1020 100%)' }} />}
      </div>
      ...
      <div className="hero-content-pad animate-fadeUp" style={{ position: 'relative', zIndex: 2, maxWidth: 860 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 20 }}>
          ...
          <span role="status" aria-live="polite" style={{ display: 'inline-flex', alignItems: 'center', gap: 6, background: 'rgba(224,80,64,0.15)', border: '1px solid rgba(224,80,64,0.4)', color: '#ff7060', fontFamily: 'var(--font-dm-mono)', fontSize: '0.52rem', letterSpacing: '0.18em', textTransform: 'uppercase', padding: '3px 10px', borderRadius: 2 }}>
        ...
        <h1 id="hero-title" style={{ fontFamily: 'var(--font-cormorant)', fontSize: 'clamp(2.4rem,5.5vw,4.4rem)', fontWeight: 700, lineHeight: 1.06, color: '#f4f6ff', marginBottom: 20, letterSpacing: '-0.01em' }}>
        ...
        <div className="hero-ctas" style={{ display: 'flex', gap: 16, alignItems: 'center', flexWrap: 'wrap' }}>
          <Link href={heroHref} target={heroTarget} rel="noopener noreferrer" className="btn-clip" style={{ background: '#f4f6ff', color: '#07080d', fontFamily: 'var(--font-dm-mono)', fontSize: '0.68rem', fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', padding: '12px 28px', textDecoration: 'none', display: 'inline-flex', alignItems: 'center', gap: 8, transition: 'background 0.15s' }}
          ...
          <a href="#nieuws" style={{ fontSize: '0.72rem', color: '#7a86a8', textDecoration: 'none', display: 'inline-flex', alignItems: 'center', gap: 6, transition: 'color 0.15s' }}
            onMouseEnter={e => (e.currentTarget.style.color = '#dde2f0')}
            onMouseLeave={e => (e.currentTarget.style.color = '#7a86a8')}
          >
            Alle nieuws
      ...
      {apod?.copyright && <div style={{ position: 'absolute', bottom: 16, right: 24, zIndex: 3, fontFamily: 'var(--font-dm-mono)', fontSize: '0.5rem', color: 'rgba(120,130,160,0.5)' }}>© {apod.copyright}</div>}
...
function TopicsStrip({ active, onFilter, counts }: { active: string; onFilter: (t: string) => void; counts: Record<string, number> }) {
  return (
    <div role="navigation" aria-label="Onderwerp filter" style={{ position: 'relative', zIndex: 1, borderBottom: '1px solid #1c2035', background: '#07080d' }}>
        ...
        {TOPICS.map(t => {
          ...
          return (
            <button
              ...
              onMouseEnter={e => { if (!isActive) e.currentTarget.style.color = '#7a86a8' }}
              onMouseLeave={e => { if (!isActive) e.currentTarget.style.color = '#4a5278' }}
            >
              {t}
              {/* Article count badge */}
              {t !== 'Alles' && count > 0 && (
                <span style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', minWidth: 18, height: 16, padding: '0 5px', background: isActive ? 'rgba(212,168,75,0.2)' : 'rgba(74,82,120,0.3)', color: isActive ? '#d4a84b' : '#4a5278', fontFamily: 'var(--font-dm-mono)', fontSize: '0.48rem', borderRadius: 2, fontVariantNumeric: 'tabular-nums' }}>
                  {count}
                </span>
              )}
              {isActive && <span aria-hidden="true" style={{ display: 'inline-block', width: 4, height: 4, borderRadius: '50%', background: '#d4a84b', marginLeft: 2 }} />}
            </button>
          )
        })}
      </div>
...
function BentoCard({ article, size }: { article: Article; size: 'hero' | 'md' | 'sm' }) {
  ...
  return (
    <article className={`card-wrap bento-${size}`} style={{ background: '#0c0e18', overflow: 'hidden', display: 'flex', flexDirection: 'column', transition: 'background 0.25s' }}
      ...
      <Link href={`/nieuws/${article.slug}`} aria-label={article.title} style={{ display: 'flex', flexDirection: 'column', flex: 1, textDecoration: 'none', color: 'inherit' }}>
        <div style={{ height: thumbH, position: 'relative', overflow: 'hidden', flexShrink: 0 }}>
          ...
          <div className="card-thumb-inner" style={{ width: '100%', height: '100%', background: gradient, position: 'relative' }}>
            ...
            {/* Category badge */}
            <span style={{ position: 'absolute', bottom: 12, left: 14, fontFamily: 'var(--font-dm-mono)', fontSize: '0.52rem', letterSpacing: '0.15em', textTransform: 'uppercase', color: article.catColor, background: 'rgba(7,8,13,0.75)', backdropFilter: 'blur(8px)', padding: '3px 8px', borderRadius: 2, border: `1px solid ${article.catColor}30`, zIndex: 2 }}>
              {article.category}
            </span>
          </div>
        </div>
        <div style={{ padding: size === 'hero' ? 32 : size === 'md' ? 20 : 16, flex: 1, display: 'flex', flexDirection: 'column' }}>
          <h2 style={{ fontFamily: 'var(--font-cormorant)', fontWeight: 700, lineHeight: 1.18, color: '#f4f6ff', fontSize: size === 'hero' ? '2rem' : size === 'md' ? '1.15rem' : '0.95rem', marginBottom: size === 'sm' ? 0 : 10 }}>
            {article.title}
          </h2>
          {size !== 'sm' && (
...
function ArticleGridCard({ article }: { article: Article }) {
  ...
  return (
    <article
      style={{ background: hovered ? '#111420' : '#0c0e18', border: '1px solid #1c2035', overflow: 'hidden', display: 'flex', flexDirection: 'column', transition: 'background 0.2s, border-color 0.2s, box-shadow 0.2s', borderColor: hovered ? article.catColor + '40' : '#1c2035', boxShadow: hovered ? `0 4px 32px ${article.catColor}18` : 'none', borderRadius: 2 }}
      ...
      <Link href={`/nieuws/${article.slug}`} aria-label={article.title} style={{ display: 'flex', flexDirection: 'column', flex: 1, textDecoration: 'none', color: 'inherit' }}>
        ...
        <div style={{ height: 140, position: 'relative', overflow: 'hidden', flexShrink: 0, background: gradient }}>
          ...
          <div style={{ position: 'absolute', top: 12, left: 12, display: 'flex', alignItems: 'center', gap: 6 }}>
            <span style={{ fontFamily: 'var(--font-dm-mono)', fontSize: '0.5rem', letterSpacing: '0.15em', textTransform: 'uppercase', color: article.catColor, background: 'rgba(7,8,13,0.8)', backdropFilter: 'blur(6px)', padding: '3px 8px', borderRadius: 2, border: `1px solid ${article.catColor}35` }}>
              {article.category}
            </span>
          </div>
          {/* Read time badge */}
          <div style={{ position: 'absolute', top: 12, right: 12 }}>
            <span style={{ fontFamily: 'var(--font-dm-mono)', fontSize: '0.48rem', color: '#7a86a8', background: 'rgba(7,8,13,0.7)', backdropFilter: 'blur(6px)', padding: '3px 7px', borderRadius: 2 }}>
        ...
        <div style={{ padding: '16px 18px 14px', flex: 1, display: 'flex', flexDirection: 'column', gap: 8 }}>
          ...
          <h3 style={{ fontFamily: 'var(--font-cormorant)', fontSize: '1.15rem', fontWeight: 700, lineHeight: 1.22, color: hovered ? '#fff' : '#f4f6ff', margin: 0, transition: 'color 0.15s', display: '-webkit-box', WebkitLineClamp: 3, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>
            {article.title}
          </h3>

          {/* Excerpt — gives the "why I should click" context */}
          <p style={{ fontSize: '0.78rem', color: '#7a86a8', lineHeight: 1.65, margin: 0, flex: 1, display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>
            {article.excerpt}
          </p>

          {/* Footer: level + date + hover arrow */}
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 4, paddingTop: 10, borderTop: '1px solid #1c2035' }}>
            <span style={{ display: 'inline-flex', alignItems: 'center', fontFamily: 'var(--font-dm-mono)', fontSize: '0.48rem', letterSpacing: '0.1em', textTransform: 'uppercase', padding: '2px 6px', background: lvlC.bg, color: lvlC.color, borderLeft: `2px solid ${lvlC.border}`, borderRadius: 2 }}>
              {LEVEL_LABEL[lvl]}
            </span>
            <span style={{ fontFamily: 'var(--font-dm-mono)', fontSize: '0.5rem', color: '#4a5278', marginLeft: 'auto' }}>
              {article.date}
            </span>
            <span style={{ fontFamily: 'var(--font-dm-mono)', fontSize: '0.5rem', color: article.catColor, opacity: hovered ? 1 : 0, transform: hovered ? 'translateX(0)' : 'translateX(-6px)', transition: 'opacity 0.15s, transform 0.15s' }} aria-hidden="true">
              → Lees
            </span>
          </div>
        </div>
...
function ISSWidget({ iss }: { iss: ISSData | null }) {
  ...
  return (
    <div role="region" aria-labelledby="iss-title" style={{ border: '1px solid #1c2035', background: '#111420', overflow: 'hidden', borderRadius: 2 }}>
      ...
      <div role="img" aria-label="ISS positie op wereldkaart" style={{ height: 160, background: '#050810', position: 'relative', overflow: 'hidden' }}>
        ...
        {iss && <div aria-hidden="true" style={{ position: 'absolute', left: `${Math.min(px + 2, 83)}%`, top: `${Math.max(py - 14, 4)}%`, fontFamily: 'var(--font-dm-mono)', fontSize: '0.48rem', color: '#3ddf90', letterSpacing: '0.1em', whiteSpace: 'nowrap', zIndex: 3 }}>ISS ↗</div>}
      </div>
...
function APODWidget({ apod }: { apod: APODData | null }) {
  return (
    <div role="region" aria-labelledby="apod-widget-title" style={{ border: '1px solid #1c2035', background: '#111420', overflow: 'hidden', borderRadius: 2 }}>
      <div style={{ padding: '11px 20px', borderBottom: '1px solid #1c2035' }}>
        <span id="apod-widget-title" style={{ fontFamily: 'var(--font-dm-mono)', fontSize: '0.58rem', letterSpacing: '0.15em', textTransform: 'uppercase', color: '#7a86a8' }}>NASA Foto van de Dag</span>
      </div>
      {apod?.media_type === 'image' ? (
        ...
        <div style={{ width: '100%', height: 160, background: 'linear-gradient(135deg,#0a1030,#1a2060)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <svg width="48" height="48" viewBox="0 0 48 48" fill="none" aria-hidden="true"><circle cx="24" cy="24" r="4" fill="rgba(212,168,75,0.6)" /><circle cx="24" cy="24" r="10" fill="none" stroke="rgba(212,168,75,0.2)" strokeWidth="1" /><circle cx="24" cy="24" r="18" fill="none" stroke="rgba(212,168,75,0.08)" strokeWidth="1" /></svg>
        </div>
      )}
      {apod && (
        <div style={{ padding: '14px 20px' }}>
          ...
          <p style={{ fontSize: '0.76rem', color: '#7a86a8', lineHeight: 1.65, display: '-webkit-box', WebkitLineClamp: 3, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>{apod.explanation}</p>
        </div>
      )}
    </div>
  )
...
function AIPromoWidget() {
  ...
  return (
    <div role="region" aria-label="AI feature" style={{ border: '1px solid rgba(138,104,32,0.5)', background: 'linear-gradient(135deg,rgba(16,14,5,0.9),rgba(26,21,8,0.9))', padding: 24, position: 'relative', overflow: 'hidden', borderRadius: 2 }}>
      ...
      <div style={{ fontFamily: 'var(--font-dm-mono)', fontSize: '0.56rem', letterSpacing: '0.18em', textTransform: 'uppercase', color: '#d4a84b', marginBottom: 12 }}>✦ AI Feature</div>
      <div style={{ fontFamily: 'var(--font-cormorant)', fontSize: '1.2rem', fontWeight: 700, color: '#f4f6ff', lineHeight: 1.2, marginBottom: 10 }}>Lees elk artikel op jouw niveau</div>
      <p style={{ fontSize: '0.76rem', color: '#7a86a8', lineHeight: 1.65, marginBottom: 16 }}>Kies Beginner, Amateur of Pro — onze AI herschrijft het artikel live voor jou.</p>
      <div role="group" aria-label="Lees niveau keuze" style={{ display: 'flex', gap: 8, marginBottom: 16 }}>
        {LEVELS_UI.map(l => (
          <button key={l.key} aria-label={`${l.label} niveau`} style={{ flex: 1, padding: '6px 0', fontFamily: 'var(--font-dm-mono)', fontSize: '0.55rem', letterSpacing: '0.08em', textTransform: 'uppercase', textAlign: 'center', border: `1px solid ${l.border}`, color: l.color, background: 'transparent', borderRadius: 2, cursor: 'pointer', transition: 'background 0.15s' }}
            onMouseEnter={e => (e.currentTarget.style.background = l.bg)}
            onMouseLeave={e => (e.currentTarget.style.background = 'transparent')}
          >{l.label}</button>
        ))}
      </div>
      <Link href="/tools/herschrijver" className="btn-clip" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#d4a84b', color: '#07080d', fontFamily: 'var(--font-dm-mono)', fontSize: '0.62rem', fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', padding: '11px 20px', textDecoration: 'none', transition: 'background 0.15s' }}
        onMouseEnter={e => (e.currentTarget.style.background = '#e8be60')}
        onMouseLeave={e => (e.currentTarget.style.background = '#d4a84b')}
      >Probeer het nu →</Link>
    </div>
...
function Newsletter() {
  return (
    <section aria-labelledby="newsletter-title" style={{ position: 'relative', zIndex: 1, background: '#0c0e18', borderTop: '1px solid #1c2035', borderBottom: '1px solid #1c2035', overflow: 'hidden' }}>
      ...
      <div className="newsletter-grid newsletter-inner" style={{ maxWidth: 'var(--max-w)', margin: '0 auto', padding: '64px var(--sp-10)' }}>
        ...
        <form action="#" method="post" aria-label="Nieuwsbrief aanmelden" style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          <div className="nl-input-wrap" style={{ display: 'flex', gap: 2, background: '#1c2035' }}>
            ...
            <input id="nl-email" type="email" name="email" placeholder="jouw@email.nl" required autoComplete="email" inputMode="email" style={{ flex: 1, background: '#0a0c14', border: 'none', padding: '14px 18px', color: '#dde2f0', fontFamily: 'var(--font-dm-mono)', fontSize: '0.7rem', outline: 'none' }}
              onFocus={e => (e.currentTarget.style.background = '#0f1120')}
              onBlur={e => (e.currentTarget.style.background = '#0a0c14')}
            />
            <button type="submit" aria-label="Aanmelden voor nieuwsbrief" style={{ background: '#d4a84b', color: '#07080d', fontFamily: 'var(--font-dm-mono)', fontSize: '0.65rem', fontWeight: 700, letterSpacing: '0.12em', textTransform: 'uppercase', padding: '14px 22px', border: 'none', cursor: 'pointer', whiteSpace: 'nowrap', transition: 'background 0.15s' }}
              onMouseEnter={e => (e.currentTarget.style.background = '#e8be60')}
              onMouseLeave={e => (e.currentTarget.style.background = '#d4a84b')}
            >Aanmelden</button>
          </div>
          <p style={{ fontFamily: 'var(--font-dm-mono)', fontSize: '0.54rem', color: '#4a5278', lineHeight: 1.6 }}>
            Door aan te melden ga je akkoord met onze <Link href="/privacy" style={{ color: '#7a86a8', textDecoration: 'underline', textUnderlineOffset: 3 }}>privacyverklaring</Link>. Je kunt je altijd uitschrijven.
          </p>
        </form>
      </div>
    </section>
    ...
    <footer role="contentinfo" style={{ position: 'relative', zIndex: 1, background: '#0c0e18', borderTop: '1px solid #1c2035' }}>
      <div className="footer-pad" style={{ maxWidth: 'var(--max-w)', margin: '0 auto' }}>
        <div className="footer-grid">
          ...
          {cols.map(col => (
            <div key={col.title}>
              <div style={{ fontFamily: 'var(--font-dm-mono)', fontSize: '0.57rem', letterSpacing: '0.18em', textTransform: 'uppercase', color: '#4a5278', marginBottom: 16 }}>{col.title}</div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                {col.links.map(([label, href]) => (
                  <Link key={label} href={href} style={{ fontSize: '0.82rem', color: '#7a86a8', textDecoration: 'none', transition: 'color 0.15s' }}
                    onMouseEnter={e => (e.currentTarget.style.color = '#dde2f0')}
                    onMouseLeave={e => (e.currentTarget.style.color = '#7a86a8')}
                  >{label}</Link>
                ))}
      ...
      <main id="main-content" tabIndex={-1} className="main-pad" style={{ position: 'relative', zIndex: 1, maxWidth: 'var(--max-w)', margin: '0 auto' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24, gap: 16 }} id="nieuws">
          <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
            <span style={{ fontFamily: 'var(--font-dm-mono)', fontSize: '0.6rem', letterSpacing: '0.22em', textTransform: 'uppercase', color: '#4a5278' }}>Uitgelicht</span>
            <div aria-hidden="true" style={{ width: 48, height: 1, background: '#252840' }} />
          </div>
          <span style={{ fontFamily: 'var(--font-dm-mono)', fontSize: '0.55rem', color: '#4a5278' }}>Laatste nieuws</span>
        </div>
          ...
          <section aria-labelledby="grid-label" aria-live="polite" aria-atomic="false">
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20, gap: 12 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
                <span id="grid-label" style={{ fontFamily: 'var(--font-dm-mono)', fontSize: '0.6rem', letterSpacing: '0.22em', textTransform: 'uppercase', color: '#4a5278' }}>
                  {activeFilter === 'Alles' ? 'Alle artikelen' : activeFilter}
                </span>
                <div aria-hidden="true" style={{ width: 32, height: 1, background: '#1c2035' }} />
              </div>
              <span style={{ fontFamily: 'var(--font-dm-mono)', fontSize: '0.54rem', color: '#4a5278' }}>
            ...
            {filtered.length === 0 && (
              <div style={{ padding: '48px 24px', textAlign: 'center', border: '1px solid #1c2035', borderRadius: 2 }}>
                <div style={{ fontFamily: 'var(--font-cormorant)', fontSize: '1.5rem', color: '#4a5278', marginBottom: 8 }}>Geen artikelen gevonden</div>
                <p style={{ fontFamily: 'var(--font-dm-mono)', fontSize: '0.62rem', color: '#2a3050' }}>Probeer een ander onderwerp of bekijk alle artikelen.</p>
                <button onClick={() => handleFilter('Alles')} style={{ marginTop: 20, fontFamily: 'var(--font-dm-mono)', fontSize: '0.6rem', color: '#d4a84b', background: 'none', border: '1px solid rgba(212,168,75,0.3)', padding: '8px 20px', borderRadius: 2, cursor: 'pointer', letterSpacing: '0.1em', textTransform: 'uppercase' }}>
                  Toon alle artikelen
                </button>
              </div>
            )}
            ...
            {hasMore && (
              <div style={{ display: 'flex', alignItems: 'center', gap: 20, paddingTop: 8 }}>
                <button
                  ...
                  style={{ fontFamily: 'var(--font-dm-mono)', fontSize: '0.62rem', fontWeight: 600, letterSpacing: '0.12em', textTransform: 'uppercase', color: '#d4a84b', background: 'none', border: '1px solid rgba(212,168,75,0.35)', padding: '11px 28px', cursor: 'pointer', borderRadius: 2, transition: 'background 0.15s, border-color 0.15s' }}
                  aria-label={`Laad meer artikelen — ${filtered.length - visibleCount} resterend`}
                  onMouseEnter={e => { e.currentTarget.style.background = 'rgba(212,168,75,0.08)'; e.currentTarget.style.borderColor = 'rgba(212,168,75,0.6)' }}
                  onMouseLeave={e => { e.currentTarget.style.background = 'none'; e.currentTarget.style.borderColor = 'rgba(212,168,75,0.35)' }}
                >
                  Laad meer
                </button>
                <span style={{ fontFamily: 'var(--font-dm-mono)', fontSize: '0.54rem', color: '#4a5278' }}>
                ...
                {/* Progress bar */}
                <div style={{ flex: 1, height: 2, background: '#1c2035', borderRadius: 1, overflow: 'hidden' }}>
                  <div style={{ height: '100%', background: '#d4a84b', width: `${(visible.length / filtered.length) * 100}%`, borderRadius: 1, transition: 'width 0.3s' }} />
                </div>
              </div>
            )}
            ...
            {!hasMore && filtered.length > 0 && (
              <div style={{ display: 'flex', alignItems: 'center', gap: 16, paddingTop: 8 }}>
                ...
                <span style={{ fontFamily: 'var(--font-dm-mono)', fontSize: '0.52rem', color: '#2a3050', flexShrink: 0 }}>Alle {filtered.length} artikelen geladen</span>
                <div style={{ flex: 1, height: 1, background: '#1c2035' }} />
              </div>
            )}
          </section>
