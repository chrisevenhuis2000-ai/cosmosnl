...
function LevelToggle({ level, loading, onChange }: {
  ...
  return (
    <div style={{ position: 'sticky', top: 60, zIndex: 30, background: 'rgba(26,26,46,0.97)', backdropFilter: 'blur(16px)', borderBottom: '1px solid #252858', padding: '0 40px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', height: 52 }}>
      <div style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: '0.6rem', letterSpacing: '0.18em', textTransform: 'uppercase', color: '#4A5A8A', display: 'flex', alignItems: 'center', gap: 8 }}>
        <span style={{ color: '#e05040' }}>◈</span> Leesniveau
      </div>
      <div style={{ display: 'flex', alignItems: 'center', background: '#16173A', border: '1px solid #252858', borderRadius: 40, padding: 3, gap: 2 }}>
        {levels.map(({ key, label, emoji }) => (
          <button key={key} onClick={() => onChange(key)} style={{
            border: 'none', borderRadius: 36, padding: '6px 16px',
            fontFamily: 'JetBrains Mono, monospace', fontSize: '0.65rem', letterSpacing: '0.08em', textTransform: 'uppercase',
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontFamily: 'JetBrains Mono, monospace', fontSize: '0.58rem', color: '#4A5A8A', minWidth: 130, justifyContent: 'flex-end' }}>
...
function ParagraphBlock({ text, isRewritten, level, loading }: {
  ...
  return (
    <p style={{
      ...
      {text || <span style={{ display: 'inline-block', width: '100%', height: '1em', background: '#252858', borderRadius: 4 }} />}
    </p>
  )
}
  ...
  if (!article) {
    return (
      <div style={{ background: '#1A1A2E', minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <div style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: '0.7rem', color: '#4A5A8A', letterSpacing: '0.2em' }}>
          LADEN...
        </div>
      </div>
    )
    ...
    <>
      ...
      <main style={{ maxWidth: 760, margin: '0 auto', padding: '56px 24px 120px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 22 }}>
          <span style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: '0.6rem', letterSpacing: '0.18em', textTransform: 'uppercase', color: col.dot, border: `1px solid ${col.border}`, padding: '3px 10px', borderRadius: 2, transition: 'all 0.3s' }}>
            {article.category}
          </span>
          <span style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: '0.6rem', color: '#4A5A8A' }}>{article.date} · {article.readTime} min lezen</span>
        </div>

        {/* Title */}
        <h1 style={{ fontFamily: 'Playfair Display, Georgia, serif', fontSize: 'clamp(1.9rem,4.5vw,3rem)', fontWeight: 900, lineHeight: 1.1, color: '#FFFFFF', marginBottom: 22 }}>
          {article.title}
        </h1>

        {/* Author */}
        <div style={{ display: 'flex', alignItems: 'center', gap: 14, paddingBottom: 24, borderBottom: '1px solid #252858', marginBottom: 28 }}>
          <div style={{ width: 40, height: 40, borderRadius: '50%', background: '#252858', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.1rem' }}>🔭</div>
          <div>
            <div style={{ fontWeight: 500, fontSize: '0.88rem', color: '#FFFFFF' }}>{article.author}</div>
            <div style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: '0.58rem', letterSpacing: '0.1em', textTransform: 'uppercase', color: '#4A5A8A' }}>{article.role}</div>
          </div>
        </div>

        {/* Level banner */}
        {level !== 'original' && (
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 16px', borderRadius: 4, marginBottom: 24, background: col.bg, border: `1px solid ${col.border}`, fontFamily: 'JetBrains Mono, monospace', fontSize: '0.63rem', letterSpacing: '0.1em', color: col.text }}>
        ...
        {article.imageUrl && (
          <div style={{ margin: '0 0 36px', position: 'relative', overflow: 'hidden' }}>
            ...
            />
            {article.imageCredit && (
              <div style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: '0.58rem', color: '#4A5A8A', padding: '8px 0', letterSpacing: '0.06em' }}>
                {article.imageCredit}
              </div>
            )}
          </div>
      ...
      {/* Footer */}
      <footer style={{ borderTop: '1px solid #252858', background: '#12132A', padding: '24px 40px', display: 'flex', justifyContent: 'space-between', fontFamily: 'JetBrains Mono, monospace', fontSize: '0.56rem', color: '#2A3060' }}>
        <span>© 2026 NightGazer</span>
        <a href="/" style={{ color: '#4A5A8A', textDecoration: 'none' }}>← Terug naar home</a>
      </footer>
    </>
  )
}